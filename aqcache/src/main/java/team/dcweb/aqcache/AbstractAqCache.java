...
public abstract class AbstractAqCache<K, V> implements AqCache<K, V> {

    private static final Logger logger = LoggerFactory.getLogger(AbstractAqCache.class);
...
//        return false;
    ...
    static <K, V> V computeIfAbsentImpl(K key, Function<K, V> loader, boolean cacheNullWhenLoaderReturnNull,
                                        long expireAfterWrite, TimeUnit timeUnit, AqCache<K, V> aqCache) {
        AbstractAqCache<K, V> abstractCache = CacheUtil.getAbstractCache(aqCache);
        CacheLoader<K, V> newLoader = CacheUtil.createProxyLoader(aqCache, loader, abstractCache::notify);
        CacheGetResult<V> r;
        if (aqCache instanceof RefreshAqCache) {
            RefreshAqCache<K, V> refreshCache = ((RefreshAqCache<K, V>) aqCache);
        ...
        if (r.isSuccess()) {
            ...
            Consumer<V> cacheUpdater = (loadedValue) -> {
                if (needUpdate(loadedValue, cacheNullWhenLoaderReturnNull, newLoader)) {
                    if (timeUnit != null) {
                        aqCache.PUT(key, loadedValue, expireAfterWrite, timeUnit).waitForResult();
                    } else {
                        aqCache.PUT(key, loadedValue).waitForResult();
                    }
                }
            };

            V loadedValue;
            if (aqCache.config().isCachePenetrationProtect()) {
                loadedValue = synchronizedLoad(aqCache.config(), abstractCache, key, newLoader, cacheUpdater);
            } else {
    ...
    static <K, V> V synchronizedLoad(CacheConfig config, AbstractAqCache<K, V> abstractCache,
    ...
    private static Object buildLoaderLockKey(AqCache c, Object key) {
        if (c instanceof AbstractEmbeddedAqCache) {
            return ((AbstractEmbeddedAqCache) c).buildKey(key);
        } else if (c instanceof ProxyAqCache) {
