...
public class CacheHandler implements InvocationHandler {
    ...
    private static void doInvalidate(CacheInvokeContext context, CacheInvalidateAnnoConfig annoConfig) {
        AqCache aqCache = context.getCacheFunction().apply(context, annoConfig);
        if (aqCache == null) {
            return;
        }
    ...
    private static void doUpdate(CacheInvokeContext context, CacheUpdateAnnoConfig updateAnnoConfig) {
        AqCache aqCache = context.getCacheFunction().apply(context, updateAnnoConfig);
        if (aqCache == null) {
            return;
        }
    ...
    private static Object invokeWithCached(CacheInvokeContext context)
        ...
        AqCache aqCache = context.getCacheFunction().apply(context, cac);
        if (aqCache == null) {
        ...
        if (key == null) {
            return loadAndCount(context, aqCache, key);
        }
        ...
        try {
            ...
            Object result = aqCache.computeIfAbsent(key, loader);
            return result;
    ...
    private static Object loadAndCount(CacheInvokeContext context, AqCache aqCache, Object key) throws Throwable {
        ...
        try {
            ...
            while (aqCache instanceof ProxyAqCache) {
                aqCache = ((ProxyAqCache) aqCache).getTargetCache();
            }
            if (aqCache instanceof AbstractAqCache) {
                ((AbstractAqCache) aqCache).notify(event);
            }
        }
        return v;
    }
    ...
    public static class CacheHandlerRefreshAqCache<K, V> extends RefreshAqCache<K, V> {

        public CacheHandlerRefreshAqCache(AqCache aqCache) {
            super(aqCache);
        }
