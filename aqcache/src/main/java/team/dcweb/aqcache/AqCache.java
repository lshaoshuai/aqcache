...
public interface AqCache<K, V> extends Closeable {

    Logger logger = LoggerFactory.getLogger(AqCache.class);
     ...
     * <p>If error occurs during cache access, the method will not throw an exception.</p>
     * <p>{@link } does not support this method.</p>
     *
     * @param key   key with which the specified value is to be associated
     * @param value value to be associated with the specified key
     ...
     *   }
     * </pre>
     * <p>{@link } will use the last level cache to support this operation.</p>
     *
     * @param key      lockKey
    ...
    default AutoReleaseLock tryLock(K key, long expire, TimeUnit timeUnit) {
        ...
        int lockCount = 0;
        AqCache aqCache = this;
        while (lockCount++ < config.getTryLockLockCount()) {
            CacheResult lockResult = aqCache.PUT_IF_ABSENT(key, uuid, expire, timeUnit);
            if (lockResult.isSuccess()) {
                ...
                while (inquiryCount++ < config.getTryLockInquiryCount()) {
                    CacheGetResult inquiryResult = aqCache.GET(key);
